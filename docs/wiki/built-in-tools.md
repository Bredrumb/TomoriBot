...
| `block_user` | n/a | `user_blocking_enabled`; active server persona context | Temporarily mute or block a Discord user for the active persona only. `mute` prevents the target from triggering that persona; `block` also hides that user's recent live dialogue-history messages/media from that persona's context. This does not forget or redact memories. |
| `unblock_user` | n/a | `user_blocking_enabled`; active server persona context | Remove an active persona-scoped mute/block for a Discord user. |
