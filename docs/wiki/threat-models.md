...
| **Information Disclosure / SSRF / Open Proxy Abuse** | A user URL or redirect targets loopback, a private network, link-local services, or cloud metadata. | High | The default `safe_http` engine validates and DNS-pins each redirect hop, caps redirects and response bytes, and blocks internal targets while `FETCH_URL_ALLOW_PRIVATE_NETWORK=false`. Custom-provider and remote-MCP HTTP calls use the same guarded fetcher. Crawl4AI requires an explicit trusted-development opt-in because its browser follows redirects outside the bot process. | `src/utils/security/userRemoteFetch.ts`, `src/tools/fetchUrl/*`, `src/utils/mcp/mcpUrlSecurity.ts` | Browser-rendered fetch output remains untrusted prompt content. Do not enable private-network fetching in production, and keep sidecar ports unpublished. |
