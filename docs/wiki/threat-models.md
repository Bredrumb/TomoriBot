...
TomoriBot supports bundled MCP servers, a REST-backed Brave engine (consumed internally by the unified `web_search` tool's dispatcher), bundled URL fetching consumed internally by the unified `fetch_url` dispatcher, an optional self-hosted SearXNG metasearch sidecar, and guild-registered remote MCP servers.
