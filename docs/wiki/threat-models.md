...
| **Information Disclosure / SSRF / Open Proxy Abuse** | The Crawl4AI sidecar is exposed publicly or allowed to fetch internal-only URLs with the bot's network position. | High | Crawl4AI is opt-in via `CRAWL4AI_BASE_URL`, hidden behind `fetch_url`, and should run only on a private compose/task network. TomoriBot health-checks the sidecar and falls back to `mcp_fetch` when unavailable. | `src/tools/fetchUrl/crawl4aiEngine.ts`, `src/tools/restAPIs/crawl4ai/*`, `docker-compose.yaml`, `servers/crawl4ai/README.md` | Browser-rendered fetch output is untrusted prompt content. Operators must keep port 11235 private, pin the image for production, and avoid exposing `CRAWL4AI_BASE_URL` as a per-guild/user setting. |
| **Information Disclosure / SSRF / Open Proxy Abuse** | The Browserless sidecar is exposed publicly or allowed to fetch internal-only URLs with the bot's network position. | High | Browserless is opt-in via `BROWSERLESS_BASE_URL`, hidden behind `fetch_url`, token-protected, and should run only on a private compose/task network. TomoriBot health-checks `/pressure` and falls back to the next engine when unavailable. | `src/tools/fetchUrl/browserlessEngine.ts`, `src/tools/restAPIs/browserless/*`, `docker-compose.yaml`, `servers/browserless/README.md` | Rendered HTML converted to markdown is untrusted prompt content. Operators must keep port 3000 private, pin the image for production, review Browserless v2 licensing, and avoid exposing `BROWSERLESS_BASE_URL` as a per-guild/user setting. |
