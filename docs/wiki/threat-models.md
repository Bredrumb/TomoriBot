...
TomoriBot supports bundled MCP servers, a REST-backed Brave engine (consumed internally by the unified `web_search` tool's dispatcher), bundled URL fetching consumed internally by the unified `fetch_url` dispatcher, optional self-hosted SearXNG, Crawl4AI, and Browserless sidecars, and guild-registered remote MCP servers.
...
| **Information Disclosure / SSRF** | Web fetch/search tools retrieve attacker-controlled pages, huge responses, internal URLs, or pages containing data-exfiltration prompts. | Medium / High | Fetch size checks, max content truncation, progress notices, feature flags, `FETCH_URL_ALLOW_PRIVATE_NETWORK=false` private-target blocking before `fetch_url` dispatch, and SSRF-protected remote fetch paths for guild MCP HTTP transports. | `src/tools/fetchUrl/*`, `src/utils/mcp/mcpExecutor.ts`, `src/tools/mcpServers/fetch/fetchHandler.ts`, `src/tools/restAPIs/brave/*` | Search/fetch output is untrusted prompt content. Built-in Brave result image fetches and third-party search APIs should be treated as external content. |
| **Elevation of Privilege / Information Disclosure** | A compromised SearXNG sidecar image makes outbound calls under the task's network/IAM identity or attempts to read other co-located container state. | High | Sidecar receives only `SEARXNG_SECRET` from Secrets Manager — never the combined `tomoribot/production` secret. Image is intended to be pinned by sha256 digest (see `servers/searxng/README.md` upgrade procedure). | `terraform/aws/ecs-health-check.tf`, `terraform/gcp/cloud-run.tf`, `servers/searxng/README.md` | Defaults in `docker-compose.yaml` / Terraform variables currently use `searxng/searxng:latest`. Production deployments must override with a pinned digest before merge, otherwise an upstream image rotation auto-deploys on the next task restart. |
...
| **Information Disclosure / SSRF / Open Proxy Abuse** | The Crawl4AI sidecar is exposed publicly or allowed to fetch internal-only URLs with the bot's network position. | High | Crawl4AI is opt-in via `CRAWL4AI_BASE_URL`, hidden behind `fetch_url`, and should run only on a private compose/task network. TomoriBot health-checks the sidecar, blocks private/internal target URLs before dispatch unless `FETCH_URL_ALLOW_PRIVATE_NETWORK=true`, and falls back to `mcp_fetch` when unavailable. | `src/tools/fetchUrl/crawl4aiEngine.ts`, `src/tools/restAPIs/crawl4ai/*`, `docker-compose.yaml`, `servers/crawl4ai/README.md` | Browser-rendered fetch output is untrusted prompt content. Operators must keep port 11235 private, pin the image for production, and avoid exposing `CRAWL4AI_BASE_URL` as a per-guild/user setting. |
| **Information Disclosure / SSRF / Open Proxy Abuse** | The Browserless sidecar is exposed publicly or allowed to fetch internal-only URLs with the bot's network position. | High | Browserless is opt-in via `BROWSERLESS_BASE_URL`, hidden behind `fetch_url`, token-protected, and should run only on a private compose/task network. TomoriBot health-checks `/pressure`, blocks private/internal target URLs before dispatch unless `FETCH_URL_ALLOW_PRIVATE_NETWORK=true`, and falls back to the next engine when unavailable. | `src/tools/fetchUrl/browserlessEngine.ts`, `src/tools/restAPIs/browserless/*`, `docker-compose.yaml`, `servers/browserless/README.md` | Rendered HTML converted to markdown is untrusted prompt content. Operators must keep port 3000 private, pin the image for production, review Browserless v2 licensing, and avoid exposing `BROWSERLESS_BASE_URL` as a per-guild/user setting. |
