...
- persona and user appearance tags are resolved from `personas.nai_tags` and `users.nai_char_tags`
...
- persona and user reference images are resolved from `personas.nai_char_ref_url` and `users.nai_char_ref_url`, normalized onto NovelAI's supported reference canvases with black padding, and sent through `director_reference_images` only for single-character generations when `NAI_ENABLE_CHAR_REFERENCES` is enabled
