...
当你的个人文本路由上每一个模型都失败时，TomoriBot 可以用服务器自己的文本模型来回答，而不是让这条消息没人回答。
这个回答走的是服务器的凭据，也计入服务器的文本配额，还会遵守服务器的消息冷却，所以每条消息都失败的提供方不会变成每条消息都有回答。报告方式和任何其他备用模型一样：一个**已用备用模型**按钮，细节里会写明是哪个模型回答的，以及在此之前发生过哪些失败。
在 `/personal config` > 模型 > 备用模型 里关掉**服务器模型备用**这一项，就能让你提供方的失败只留在你这边。
这个设置是账号级的，默认开启，所以它会跟着你到每一个允许它的服务器。
...
你触发的消息需要先有个人提供方，她才能回答。个人文本路由失败了就一直失败：不向成员提供自己模型的服务器，也不会借出一个模型来当备用。个人提供方在你使用她的每一个服务器里都生效。
:::
...
- `/personal config`：允许服务器的文本模型为失败的个人文本路由兜底，或关掉它。
