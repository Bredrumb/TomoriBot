...
重复同一个 emoji 可以增强效果。Emoji 控制并不完全一致，因此更适合作为风格提示，而不是保证得到某种输出。完整的上游列表和后续更新请参阅 [IrodoriTTS 官方 emoji 标注](https://huggingface.co/Aratako/Irodori-TTS-v4.1-Small/blob/main/EMOJI_ANNOTATIONS.md)。
