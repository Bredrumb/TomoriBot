...
| [ChatMock](./setup-chatmock) | Local OpenAI-compat bridge for Codex CLI |
...
|---------|------|---------|-------|
| **SearXNG** | `web_search` | Privacy-respecting metasearch engine proxy to avoid rate limits | [Setup Guide](./setup-searxng) |
| **Crawl4AI** | `fetch_url` | Browser-rendered markdown extraction for JS-heavy sites | [Setup Guide](./setup-crawl4ai) |
...
# With a local TTS server (venv must be set up first — see docs/self-hosting/text-to-speech/)
...
Docker sidecars (`--searxng`, `--crawl4ai`) are created on first run and reused on subsequent runs. Python TTS/STT sidecars require their venv to be set up once beforehand; see the individual setup guides in `docs/self-hosting/text-to-speech/` and `docs/self-hosting/speech-to-text/`.
