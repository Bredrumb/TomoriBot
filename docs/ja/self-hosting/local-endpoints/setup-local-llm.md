...
## 4.（オプション）ComfyUIとGPUを共有する

テキストモデルとComfyUIが同じGPUを使う場合、画像または動画の生成中にTomoriBotがテキストモデルをアンロードできます。生成中に届いたテキストの返信は、GPUが空くまで待機します。

保存したエンドポイントを開き、`エンドポイントを編集`を押して、`エンドポイントの動作`にある`ComfyUIのジョブ中はモデルをアンロード`を有効にします。保存時にTomoriBotがURLのサーバーを確認します。

- **Ollama：** 追加設定は不要です。生成前にモデルをアンロードし、次のテキスト要求時にOllamaが再読み込みします。
- **KoboldCpp：** `--admin`と`--admindir`を指定して起動し、管理者パスワードをエンドポイントの認証トークンに設定します。TomoriBotは管理APIでモデルをアンロードし、生成後に再読み込みします。

その他のサーバーは保存時に拒否されます。OpenAI互換APIには、モデルをアンロードする共通の方法がありません。
...
- **Composeを使用する場合：** `RUN_ENV=development`ではローカルHTTPエンドポイントを使用できます。本番環境ではHTTPSが必須になり、明示的な許可がない限りプライベートネットワークへの接続が制限されます。`FETCH_URL_ALLOW_PRIVATE_NETWORK=true`は本番環境の`fetch_url`にのみ適用され、プロバイダーのエンドポイント設定は変更しません。クラウドのメタデータアドレスはどの環境でもブロックされます。
