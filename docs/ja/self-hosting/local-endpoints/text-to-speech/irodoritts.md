...
設定した最小文字数を超えたあと、日本語と英語の句読点、カンマ、改行を分割候補として扱います。十分に長い文章でも分割候補の記号がなければ、1回の合成リクエストのままになる場合があります。

参照音声を使わないVoiceDesignでは、最初のチャンクで実際に使用されたIrodoriのseedを後続チャンクでも再利用し、チャンク間の声質変化を抑えます。参照音声モードでは、同じ参照クリップを各チャンクへ適用します。
...
| `IRODORI_CHUNK_MIN_CHARS` | `80` | 句読点で分割可能になるまでのおおよその非空白文字数 |
