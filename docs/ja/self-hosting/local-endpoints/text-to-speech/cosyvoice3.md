...
ラッパーは通常の`tts-clone`フィールドを受け付けます。
...
1つを期待するため、このローカルサーバーは完全なWAVを返却し、上流の推論は既定で`stream=False`とします。
...
または、設定済みのローカルサーバーとTomoriBotを一緒に起動します。
...
上流の依存関係は現在、CUDA 12.1のパッケージインデックスを使うPyTorch 2.3.1、Linux上のCUDA 12対応ONNX Runtimeパッケージ、Linux上のTensorRT 10.13パッケージを使用します。より新しいPyTorch CUDAビルドを必要とするハードウェアを使用している場合は、上流の依存関係をインストールした後にローカルサーバーのvenv内へ互換性のあるPyTorchビルドをインストールし、お使いのドライバーでテストしてください。
...
これらは通常のPyTorchサーバーが動作するようになってから使用してください。Discordの音声メッセージという用途では、すでに小型な0.5Bモデルを最適化するよりも、余計なランタイムの複雑さを避ける方が普通は有用です。
