...
Chatterboxでは、クリップを長くしても意味はありません。Chatterboxのランタイムは、デコーダー条件では参照を最初の10秒に、エンコーダー条件では最初の6秒に切り詰めます（[`tts.py`](https://github.com/resemble-ai/chatterbox/blob/master/src/chatterbox/tts.py)）。そのため、約10秒を超えるクリップはアップロードされ、保存された後に一部が無視されます。話者埋め込みは依然としてクリップ全体から計算されます。
