...
Related reference:
- `docs/en/architecture/subsystems/security.md`
- `docs/en/architecture/pipelines/tool-loop/` (tool-loop pipeline reference)
- `docs/en/architecture/integrations/matrix/bridge.md`
- `docs/en/architecture/integrations/voice/README.md`
...
| **Tampering / Stale State** | Caches continue serving old permissions, config, memories, webhooks, or Matrix links after a write. | Medium | Cache invalidation is implemented near successful write paths for key state. | `src/utils/cache/*`, `src/utils/bridges/matrix/*` | New DB writes must invalidate matching caches only after successful writes. |
...
| **Spoofing** | A foreign webhook or bot message is treated as TomoriBot/persona output, poisoning reply attribution or causing response loops. | High | Triggers ignore standard bot messages. Foreign bot messages are loaded into context as normal users. Webhook attribution relies on display names, meaning a foreign webhook sharing a persona name is treated as persona output in history. | `src/utils/chat/admission.ts`, `src/utils/chat/contextPipeline.ts`, `src/utils/chat/replyDecision.ts` | Unauthorized webhooks can inject persona history due to display-name matching. Admins must restrict webhook creation. |
...
| **Tampering** | LLM-driven `create_thread` creates unwanted channels of conversation. | Medium | Runtime requires `thread_creation_enabled`, bot `CreatePublicThreads` and `SendMessagesInThreads`, and target-channel visibility for the invoking member. Target resolution fails closed on ambiguous names and does not create threads inside threads. | `src/tools/functionCalls/createThreadTool.ts`, `src/utils/discord/targetResolver.ts` | The thread is created with bot permissions. Server admins should disable the feature flag where thread creation should stay manual. |
| **Information Disclosure** | `cross_channel_message` peeks into a channel and leaks recent messages back to another channel. | High | Runtime requires bot `ViewChannel`, requires the invoking guild member to have `ViewChannel` on the target, and dispatch requires bot `SendMessages` or `SendMessagesInThreads`. The server cross-channel blocklist blocks configured channels and forum/media parents. | `src/tools/functionCalls/crossChannelMessageTool.ts`, `src/utils/discord/interactions/configChannelRoutes.ts` | The bot can still expose messages from channels visible to the invoker and bot. Admins should block sensitive channels where cross-channel tool use is unwanted. |
...
| **Tampering / Information Disclosure** | RAG, server memories, personal memories, imported memories, documents, or SillyTavern presets poison future context or leak sensitive content to later generations. | High | Memory scopes are separated, personal privacy levels can block personal memory use, server/user limits exist, and management/export/remove commands are scoped. | `src/utils/db/memoryLimits.ts`, `src/commands/memory/*`, `src/commands/personal/config.ts`, `src/utils/text/contextBuilder.ts` | Server-wide memory is inherently shared. Moderators must remove poisoned or sensitive server memory. |
...
TomoriBot supports bundled MCP servers, a REST-backed Brave engine (consumed internally by the unified `web_search` tool's dispatcher), bundled URL fetching consumed internally by the unified `fetch_url` dispatcher, optional self-hosted SearXNG and Crawl4AI servers, and guild-registered remote MCP servers.
...
| **Information Disclosure / SSRF** | Web fetch/search tools retrieve attacker-controlled pages, huge responses, internal URLs, or pages containing data-exfiltration prompts. | Medium / High | Fetch size checks, max content truncation, progress notices, feature flags, production private-target blocking before `fetch_url` dispatch (enforced when `RUN_ENV=production` unless `FETCH_URL_ALLOW_PRIVATE_NETWORK=true`; auto-relaxed outside production), and SSRF-protected remote fetch paths for guild MCP HTTP transports. | `src/tools/fetchUrl/*`, `src/utils/mcp/mcpExecutor.ts`, `src/tools/restAPIs/brave/*` | Search/fetch output is untrusted prompt content. Built-in Brave result image fetches and third-party search APIs should be treated as external content. |
| **Elevation of Privilege / Information Disclosure** | A compromised SearXNG container image makes outbound calls under the task's network/IAM identity or attempts to read other co-located container state. | High | The container receives only `SEARXNG_SECRET` from Secrets Manager: never the combined `tomoribot/production` secret. The retained legacy GCP Terraform disables the container by default. Image is intended to be pinned by sha256 digest when enabled (see `servers/searxng/README.md` upgrade procedure). | `terraform/aws/ecs-health-check.tf` and `terraform/gcp/cloud-run.tf` (`release` branch), `servers/searxng/README.md` | Defaults in `docker-compose.yaml` and the optional SearXNG image variable use `searxng/searxng:latest`. Production deployments that enable SearXNG must override with a pinned digest before merge, otherwise an upstream image rotation can auto-deploy on the next task restart. |
| **Information Disclosure / Open Proxy Abuse** | The SearXNG container is exposed beyond the private container network (e.g. added to Cloud Run public ingress or a public ALB), turning it into an open metasearch proxy. | High | `servers/searxng/limiter.toml` disables bot detection on the assumption that only TomoriBot on the private network talks to the container. Deployment configs bind SearXNG to loopback (`http://localhost:8080/`) inside the task / pod and never publish port 8080. | `servers/searxng/limiter.toml`, `docker-compose.yaml`, `terraform/aws/ecs-health-check.tf` and `terraform/gcp/cloud-run.tf` (`release` branch) | If operators expose port 8080 publicly, anonymous internet traffic can use the instance to scrape Google/Bing/Brave on the operator's behalf. Never add SearXNG to public ingress. Never expose `SEARXNG_BASE_URL` as a per-guild slash-command setting: that would make it a user-controlled URL and require `fetchUserRemoteUrl()`. |
| **Information Disclosure / SSRF / Open Proxy Abuse** | A user URL or redirect targets loopback, a private network, link-local services, or cloud metadata. | High | The default `safe_http` engine validates and DNS-pins each redirect hop, caps redirects and response bytes, and in production blocks internal targets unless `FETCH_URL_ALLOW_PRIVATE_NETWORK=true` (the guard auto-relaxes outside production, `RUN_ENV` != `production`). Cloud instance-metadata / link-local addresses (`169.254.0.0/16` incl. `169.254.169.254`, IPv6 link-local, AWS IPv6 IMDS) are blocked by an always-on denylist (`src/utils/security/cloudMetadata.ts`) that neither the dev auto-relax nor the opt-in can bypass, in both the pre-dispatch gate (`urlSafety.ts`) and the connection/redirect gate (`remoteUrlSecurity.ts`). Custom-provider and remote-MCP HTTP calls use the same guarded fetcher. Crawl4AI is only admitted where private-network fetching is permitted because its browser follows redirects outside the bot process; the pre-dispatch metadata denylist is its sole in-bot SSRF check. | `src/utils/security/userRemoteFetch.ts`, `src/tools/fetchUrl/*`, `src/utils/security/remoteUrlSecurity.ts` | Browser-rendered fetch output remains untrusted prompt content. Do not enable private-network fetching in production, and keep local server ports unpublished. |

---

## 5. Custom Provider, Endpoint, Speech, and Transcription Surfaces

Custom endpoints include text, embedding, image, video, speech/TTS, and transcription/STT capabilities.
Server-scoped endpoints are admin configured and personal endpoints are user scoped. Both use the same
environment-controlled remote URL policy: local/private development endpoints are allowed outside production,
while production blocks loopback/private targets and requires the production protocol policy.

| Threat (STRIDE) | Scenario | Risk | Current Mitigation | Implementation Area | Residual Risk / Assumption |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Information Disclosure / SSRF** | Custom endpoint URL targets internal infrastructure or changes DNS after validation. | High | `/providers` and `/personal providers` probe registrations through `validateCustomEndpointReachability()`, which uses `fetchUserRemoteUrl()`. Runtime adapters use the same guarded fetcher, revalidate redirects, resolve before sending, and pin requests to validated addresses. | `src/utils/discord/interactions/providersRoutes.ts`, `src/utils/provider/providerPanelOperations.ts`, `src/utils/provider/customEndpointService.ts`, `src/providers/custom/*`, `src/utils/security/userRemoteFetch.ts` | Localhost and private endpoints are supported for local development. Production must keep `RUN_ENV=production`; neither owner scope bypasses the production blocklist. |
...
| **Tampering / DoS** | ComfyUI or custom image/video workflow JSON is malicious, huge, or unexpectedly expensive. | Medium | Workflow JSON is provided while registering or editing a model and stored in endpoint config. Generation is gated by image/video feature flags and quotas. | `src/utils/discord/interactions/providersRoutes.ts`, `src/utils/provider/customEndpointService.ts`, `src/providers/custom/customEndpointDispatcher.ts`, `src/providers/custom/customComfyUiEndpoint.ts`, `src/utils/quota/*` | The remote workflow runtime is outside the bot's sandbox. Endpoint operators own that risk. |
...
| **Information Disclosure** | `read_file`, image analysis, or GIF processing exposes attachments from recent messages. | Medium | Tools use recent-message windows and opaque `media_N` handles. | `src/tools/functionCalls/readFileTool.ts`, `src/tools/functionCalls/analyzeImageTool.ts`, `src/tools/functionCalls/processGifTool.ts` | These tools operate with bot channel visibility. Sensitive attachments in channels visible to the bot may be sent to the configured model/provider. |
...
| Threat (STRIDE) | Scenario | Risk | Current Mitigation | Implementation Area | Residual Risk / Assumption |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Spoofing / Elevation of Privilege** | Unauthorized callers post Matrix appservice transactions to TomoriBot. | Critical | `matrix-appservice-bridge` registration uses `MATRIX_HS_TOKEN` for homeserver-to-appservice authentication. Public callback URL must be HTTPS unless localhost. | `src/utils/bridges/matrix/events.ts` | Deployment must not expose the appservice listener without the homeserver auth path. Keep `MATRIX_HS_TOKEN` secret. |
| **Information Disclosure** | A Matrix room is linked to a sensitive Discord channel by mistake. | High | `/matrix link` requires `ManageGuild`, checks room encryption state, and stores explicit room-channel links. | `src/commands/matrix/link.ts`, `src/utils/bridges/matrix/rooms.ts` | Once linked, Matrix room members can interact with the Discord channel through the bridge. Room membership is controlled on the Matrix side. |
| **Information Disclosure** | E2EE Matrix rooms are linked, causing unreadable or misleading bridge behavior. | Medium | Link command checks for `m.room.encryption` and blocks encrypted rooms. | `src/utils/bridges/matrix/rooms.ts`, `src/commands/matrix/link.ts` | If the encryption check fails, code proceeds optimistically per comments; operators should keep linked rooms unencrypted. |
| **Spoofing** | Remote Matrix users choose names resembling personas or Discord users. | Medium | Discord webhook username includes `[Matrix|@user:server] localpart`; bridge user IDs are tracked in context. Virtual persona loop prevention checks the configured homeserver suffix. | `src/utils/bridges/*`, `src/utils/bridges/matrix/*`, `src/events/messageCreate/tomoriChat.ts` | Human readers can still be fooled by display names. Treat Matrix identity as Matrix ID plus homeserver, not localpart alone. |
| **Denial of Service** | Matrix media events relay large files into Discord or exhaust memory. | Medium | Media relay enforces `MATRIX_MAX_ATTACHMENT_MB`, timeout, content-length, and final buffer size checks. | `src/utils/bridges/matrix/media.ts`, `src/events/messageCreate/matrixRelay.ts` | Files are proxied through the configured homeserver using appservice auth. A hostile homeserver can still pressure the bridge within configured limits. |
| **Tampering / DoS** | Any Matrix user in a linked room issues `/kill` or `/refresh`. | Medium | Commands are limited to linked Matrix rooms and affect only the linked Discord channel. | `src/utils/bridges/matrix/events.ts` | These Matrix text commands are not Discord-permission-gated. Matrix room moderation is the control. |
...
| **Information Disclosure** | CI/CD secrets leak through logs or compromised actions. | High | Production deployment uses OIDC where documented and avoids long-lived static AWS credentials where possible. | `.github/workflows/*`, `docs/en/architecture/subsystems/security.md` | GitHub Actions and cloud IAM configuration remain part of the trust boundary. |
