...
4. Add locale keys to `src/locales/en-US/`; other locales fall back to English, so translating them is optional. Command keys live in `src/locales/{locale}/commands/{category}.ts`. Other key types live in the matching top-level file (`general.ts`, `tools.ts`, etc.) under `src/locales/{locale}/`.
