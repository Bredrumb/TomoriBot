...
| Create / update task | — | Schedule or edit reminders and self-tasks (see [Scheduled Tasks](/features/scheduled-tasks/)). |
...
| Read document | — | Extract text from a PDF/TXT/MD attachment. |
...
| Generate image / anime image | `imagegen_enabled` + capable provider | Generate or edit images (see [Media Generation](/features/media-generation/)). |
...
[Setup: Local MCP Server](/self-hosting/setup-local-mcp/).
...
[Chatting & Triggers](/features/chatting-and-triggers/#deliberate-trigger-mode). Both are
