...
- **`presence_seen` is behavioral, not telemetry.** It is a two-phase write owned by `@/utils/chat/reunionPresence`: the scope is resolved at *context build* (`resolveReunionNotes`) and committed post-turn (`recordReunionPresence`), so the read never sees its own tick and a turn that produced no response records nothing. It is the only metric recorded **in DMs** — because it answers "when did this persona last see this person", which drives reunion notes (see [dialogue history](/architecture/pipelines/context-build/02-native-assembly/11-dialogue-history/)). One tick per turn per human present in the context window, keyed to the answering persona's lineage. It is deliberately *not* `message_sent`: that metric's guild-only / successful-turn / triggerer-only rules exist for leaderboard correctness, and reusing it both broke reunion timing and would have inflated every read that sums it. No `/stats` read surfaces `presence_seen`.
