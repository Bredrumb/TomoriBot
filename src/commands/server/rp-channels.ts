...
const MODAL_CUSTOM_ID = "server_rp_channels_modal";
const CHECKBOX_ID_PREFIX = "server_rp_channels_checkbox_group";
const PAGE_BUTTON_PREFIX = "server_rp_channels_page_";
const DONE_BUTTON_ID = "server_rp_channels_done";

export const configureSubcommand = (subcommand: SlashCommandSubcommandBuilder) =>
  subcommand.setName("rp-channels").setDescription(localizer("en-US", "commands.server.rp-channels.description"));
...
export async function execute(
  ...
  try {
    ...
    if (availableChannels.length === 0) {
      await replyInfoEmbed(interaction, locale, {
        titleKey: "commands.server.rp-channels.no_channels_title",
        descriptionKey: "commands.server.rp-channels.no_channels_description",
    ...
    await log.error("Error in /server rp-channels command", error, context);
...
async function executeSinglePage(
  ...
  const modalResult = await promptWithRawModal(
    ...
    locale,
    {
      ...
      modalTitleKey: "commands.server.rp-channels.modal_title",
...
async function executeMultiPage(
  ...
  if (totalPages > CHECKLIST_MAX_PAGE_BUTTONS) {
    await replyInfoEmbed(interaction, locale, {
      titleKey: "commands.server.rp-channels.too_many_pages_title",
      descriptionKey: "commands.server.rp-channels.too_many_pages_description",
      descriptionVars: {
  ...
  await interaction.reply({
    ...
    components: buildChecklistPageActionRows(
      ...
      localizer(locale, "commands.server.rp-channels.done_button"),
      PAGE_BUTTON_PREFIX,
      DONE_BUTTON_ID,
    ),
    flags: MessageFlags.Ephemeral,
  ...
  while (true) {
    ...
    try {
      ...
      log.info("[RpChannels] Page selection timed out");
      break;
    }
...
function buildCheckboxGroups(
  ...
  return buildChannelCheckboxGroups({
    ...
    labelKey: "commands.server.rp-channels.checkbox_label",
    labelKeyContinued: "commands.server.rp-channels.checkbox_label_continued",
    descriptionKey: "commands.server.rp-channels.checkbox_description",
  });
}
...
function buildPageSelectEmbed(locale: string, channelCount: number, totalPages: number, selectedCount: number) {
  return createStandardEmbed(locale, {
    titleKey: "commands.server.rp-channels.select_page_title",
    descriptionKey: "commands.server.rp-channels.select_page_description",
    descriptionVars: {
...
async function persistUpdate(
  ...
  if (enabledIds.length === 0 && disabledIds.length === 0) {
    await replyInfoEmbed(responseInteraction, locale, {
      titleKey: "commands.server.rp-channels.no_changes_title",
      descriptionKey: "commands.server.rp-channels.no_changes_description",
      color: ColorCode.INFO,
    });
  ...
  await replyInfoEmbed(responseInteraction, locale, {
    titleKey: "commands.server.rp-channels.success_title",
    descriptionKey: "commands.server.rp-channels.success_description",
    descriptionVars: {
