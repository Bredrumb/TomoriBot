...
export class GenerateImageTool extends BaseTool {
  name = "generate_image";
  description =
    "Generate an AI image using the active provider's native image model. Use this only when the user explicitly asks you to make, draw, generate, create, edit, or continue an image; do not call it for casual visual discussion. If the user asks you to make/draw/generate an image and this tool is available, call this tool instead of only describing the image. Provide a detailed text prompt describing what image you want to create. For text-to-image requests, creatively expand sparse user ideas into one coherent finished image with a clear subject, setting, pose, outfit, colors, mood, and lighting. If you provide a media_id or target_identity reference for normal image-to-image, treat the reference as loose inspiration and describe the desired new similar image with the requested changes clearly visible. Use inpainting instead for precise localized edits that should preserve most of the source image. For inpainting, keep the prompt tightly scoped to the masked region only and do not re-describe the whole subject/reference image. For example, use 'change hair to blonde with soft highlights' instead of a full character/scene description. For background-mode inpainting, describe the desired new surroundings explicitly, including the target background color, environment, location, atmosphere, lighting, or setting, and name any old surroundings that should not remain. The mask_prompt must be a short phrase naming the existing region to mask, not the desired replacement. You can also specify an aspect ratio (default is 1:1). After generating, the image will be sent directly to the Discord channel.";
