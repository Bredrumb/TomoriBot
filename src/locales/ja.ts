...
- STMは時間とともに自動的に期限切れになります`,
        tagging_title: `記憶タグ付け`,
        tagging_description: `{memoryTaggingSet}で記憶タグとチャンネルタグを有効にできます。

**キーワード：**
キーワードを使って特定の記憶を呼び起こすことができます！デフォルトでは毎回すべての記憶をプロンプトに送りますが、キーワードを設定するとトークン節約になるほか、ロールプレイで何をいつ出すかコントロールするのにも便利です。サーバー内の他のユーザーを驚かせたり、重要な場面まで世界観の設定を秘密にしておいたりできます。記憶にキーワードを設定すると、そのキーワードがプロンプトに含まれている時だけ「思い出す」ようになります。ユーザー名やチャンネル名はヒットしすぎるので避けてください。キーワードのない記憶は常に有効です。

**チャンネルタグ：**
どの*チャンネルで*思い出すかも制御できます！記憶に #チャンネル名 のタグを付けると、そのチャンネルでのみ思い出すようになります。本格的なロールプレイ向けの機能で、チャンネルごとに異なるAU・タイムトラベル・フラッシュバックなどの世界観を設定できます。同じサーバーで複数のユーザーがそれぞれ別の冒険を楽しんでいても、チャンネルごとに異なる設定を持たせることができます。チャンネルタグのない記憶はどのチャンネルでも有効です。

キーワードとチャンネルタグは組み合わせて使うこともできます！また、デフォルトの動作は変わりません——どのプロンプトでも、どのチャンネルでも常に思い出してほしい場合は、タグを付けないだけでOKです。

*自己ホストでRAGと埋め込みが有効な場合、チャンネルタグはドキュメント（\`/memory document add\`）や履歴インポート（\`/memory history import\`）にも適用でき、世界観の設定をより強固に固定できます。*`,
      },
      spotlight: {
