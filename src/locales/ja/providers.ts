...
{failure_list}`,
    ...
    tips: {
      ...
      api_key_rotation: `このサーバーで複数のキーをお持ちの場合は、\`/provider api-key rotation\` を設定してキーを順番に使用できます。`,
      model_fallback: `耐障害性を高めるには、\`/model fallback\` でバックアップモデルを追加してください。`,
      model_fallback_personal: `耐障害性を高めるには、\`/personal model fallback\` でバックアップモデルを追加してください。`,
      ...
      choose_supported_model: `サポートされているモデルIDを \`/model text\` またはカスタムエンドポイント設定で選択してください。`,
      choose_supported_model_personal: `サポートされているモデルIDを \`/personal provider model-text\` またはカスタムエンドポイント設定で選択してください。`,
      verify_api_key: `このサーバーのAPIキーをもう一度確認してから、再試行してください。`,
      verify_api_key_personal: `\`/personal provider add\` で個人APIキーをもう一度確認してから、再試行してください。`,
      ...
      reduce_output_tokens_personal: `\`/personal parameters\`（出力トークン）で応答の長さの上限を下げると、会話履歴のための余裕を増やせます。`,
      ...
      openrouter_add_credits_personal: `[OpenRouterクレジット](https://openrouter.ai/settings/credits)でクレジットを追加するか、\`/personal parameters\`（出力トークン）で応答の長さを下げてください。`,
      ...
      adjust_parameters: `\`/model parameters\` を使用し、**Temperature** または **Top P** のどちらか一方のみが送信されるように調整してください。`,
      ...
      disable_personal_text_override: `このサーバーの設定モデルを使いたい場合は、\`/personal provider toggle-models\` で **テキスト** を無効にしてください。ユーザーBYOKモードのサーバーでは、引き続き個人プロバイダーが必要な場合があります。`,
