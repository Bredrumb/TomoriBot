...
- {personaStmEdit}：現在のチャンネルでペルソナのライブSTMを手動編集します。`,
    ...
    "memory-tagging": {
      ...
      title: `記憶タグ付け`,
      embed_description: `まずスコープによる絞り込みが働きます：サーバー記憶が届くのはそのサーバー内のプロンプトだけ、個人記憶が届くのはそのユーザーが会話に現れているときだけ、そしてどちらもそれを所有するペルソナに対してのみです。そのスコープの中では、デフォルトですべての記憶が毎回のプロンプトに含まれます。タグを使うとさらに絞り込めます。{memoryTaggingSet}で記憶タグとチャンネルタグを有効にできます。`,
