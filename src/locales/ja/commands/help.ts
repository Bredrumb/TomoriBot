...
- 表示用の文字起こし投稿は{configSpeechTranscripts}で個別に制御されます`,
    ...
    getting_started: {
      ...
      get_api_key: {
        title: `APIキーの取得`,
        description: `APIキーにより、私はAIプロバイダーのモデルにアクセスできます。
生成するものはすべてそのキーに請求されるため、
他のパスワードと同様に扱ってください。
セットアップを完了するには、キーが1つ必要です：
> **1.** 下からプロバイダーを選んでガイドを開きます。
> **2.** 表示されたキーをコピーします。**絶対に共有しないで**
> ください。また、チャンネルに貼り付けず、{setup}が
> 開く入力ボックスにのみ貼り付けてください。
> **3.** {setup}を実行し、求められたらキーを貼り付けます。`,
        picker_footer: `-# 2つ目のリストはオプションです。Brave Searchはウェブ結果を追加し、
-# ElevenLabsはホスト型音声を追加します。どちらもセットアップ完了には不要です。
-# 目的のエンドポイントがリストにない場合は、キー設定をスキップして、
-# このページの **カスタムエンドポイント（高度）** を読んでください。`,
      },
      change_trigger_behavior: {
        title: `トリガー動作の変更`,
        description: `初期状態では、あなたが私の名前を呼ぶか、メンションするか、
返信した時に応答します。チャンネルごとにこの条件を広げたり、
狭めたり、オフにしたりできます。

**私が発言を許可される場所**
ホワイトリストに登録されたチャンネルでのみ応答させることができます。
{moderationWhitelist} で追加してください。
> リストにないチャンネルでは沈黙を保ちます。

**自分から発言する**
{configAutoTrigger} を使うと、数メッセージごとに、
またはランダムに自動でメッセージを送信できます。

**メンションでのみトリガー**
意図的なトリガーモードでは、単に名前を呼ばれただけでは応答しません。
@メンションを待つようになります。
{configBehaviorTrigger} でオンにできます。`,
      },
      create_first_persona: {
        title: `最初のペルソナの作成`,
        description: `*ペルソナ*は、別の名前、アバター、性格などを持ちながら、
同じ機能を備えた私です！ 既存のペルソナを編集するか、
新しいペルソナを作成できます。

**既存のペルソナを変更する**
{configPersonaGeneral}で名前、性格、呼び方を設定します。
{configPersonaAppearance}でアバターを設定します。

**1つの文章から新しく作成する**
{personaGenerate}は短い説明からペルソナ全体を構築します。
{personaCreate}は空白のテンプレートを提供します。

**別の場所から取り込む**
{personaImport}はbotbooruやchubなどのカードサイトから
ダウンロードしたキャラクターカードを受け入れます。`,
        footer: `一度に複数のペルソナを保持できます。機能の
-# **複数のペルソナ** を参照してください。`,
      },
      explore_features: {
        title: `機能を探索する！`,
        description: `セットアップが完了しました。話せるようになった私にできることは以下の通りです。
- {expressionsInitialize} を実行すると、**このサーバーの絵文字とスタンプを使用**できます。
- {configWelcome} から**新しいメンバーに挨拶**します。
- **記憶とリマインダー**: リマインドを頼むか、
  覚えておく価値のあることを私に伝えてください。
- **ウェブ検索**などの機能は、{configTools}でオンにできます。
- {generateImage}、{generateVideo}、{generateVoice}で
  **画像、動画、音声を作成**できます。

私のスイッチのほとんどは{config}にありますが、すべてではありません。`,
        footer: `**Brave Search**は、私が既に行う検索にウェブ結果を追加します。
-# 独自のキーが必要で、セットアップの下の **APIキーの取得** から
-# オプションサービスとしてガイドを開くことができます。
-# ドキュメントサイトはこのパネルの完全版で、すべての設定を詳しく説明しています。`,
      },
    },
    personal_profile: {
      title: `個人プロフィール`,
      description: `あなたに属し、私がいるすべてのサーバーに\n引き継がれる設定です。`,
      nickname_pronouns: {
        title: `あなたのニックネームと代名詞`,
        description: `私のすべての場所でのあなたの呼び方を教えてください。\n\n{personalProfile}で設定します。\n> ニックネーム: Discord名の代わりに私があなたを呼ぶ名前\n> プレフィックスとサフィックス: \`-san\`のような称号や敬称\n> 代名詞: \`彼女/彼女の\`、\`彼ら/彼らの\`、\`任意\`、またはあなたの名前\n\nフィールドを空白のままにすると、現在のDiscord名と\nペルソナ自身の命名の癖にフォールバックします。`,
        footer: `単一のペルソナが別の呼び方をすることもできます。それは\n-# プロフィール > ペルソナ固有の環境設定で設定します。`,
      },
      personal_memories: {
        title: `個人の記憶`,
        description: `すべてのサーバーで、あなたについて具体的に記憶していること。\n\n**話しかけるだけ**\nチャットで言えば、自分で保存します。このとき、\n確認メッセージが表示されます。\n\n**または手動で管理する**\n{personalMemories}は私が保持しているあなたに関するすべてをリストし、\n個々のエントリを編集または削除できます。\n\n**私が使用できる量を決定する**\n{personalPrivacy}は、完全なパーソナライズから\nパーソナライズなしまで、プライバシーレベルを設定します。`,
        footer: `これらは、ここの誰でも閲覧および編集できる\n-# サーバーの共有メモリとは別物です。`,
      },
      personal_providers: {
        title: `個人用プロバイダー（高度）`,
        description: `あなたが私に話しかけるすべての場所で、サーバーが使用するもの\nの代わりに、あなた自身のAPIキーとモデルで応答します。\n\n**プロバイダーを保存する**\n{personalProviders}はあなたのキーを保存し、\nすぐに個人用テキストモデルをオンにします。\n\n**別のモデルを選ぶ**\n{personalModels}でモデルを切り替え、サンプラーと\nフォールバックも同じカテゴリにあります。\n> 個人設定は、あなたがトリガーした応答にのみ影響します。\n> サーバー内の他の誰にも切り替わりません。\n\nこれを必須とするサーバーもあります。ユーザーBYOKがオンの場合、\nここでプロバイダーを保存するまで応答できません。`,
      },
    },
    custom_endpoints: {
      title: `カスタムエンドポイント（高度）`,
      description: `Ollama、LM Studio、LiteLLM、KoboldCPP、ComfyUI、または\nセルフホストの音声サーバーなど、あなたが運用または信頼する\nエンドポイントを指定します。\n> {providers} はサーバー全体に登録します。\n> {personalProviders} はあなた専用に登録します。`,
      text_models: {
        title: `テキストモデル`,
        description: `**新しいカスタムエンドポイントを追加** を選択し、ラベル、\nベースURL、APIスタイルを入力します。必要に応じて\n認証トークンを追加します。\n\n保存したラベルを選択し、**+ 新しいテキストモデルを追加** を選び、\nエンドポイントが想定する正確なモデルコードを入力します。モデルを\n追加すると有効になります。\n> ビジョン、ツール使用、構造化出力を正確に申告してください。\n> 何を送信するか決定する際にこれらのフラグを信頼します。\n\n後で {configSwitchModels} から切り替えることができます。`,
        footer: `完全なAPIスタイルと互換性のリファレンスは、\n-# ドキュメントサイトにあります。`,
      },
      comfyui: {
        title: `ComfyUI（動画と画像用）`,
        description: `まずComfyUIでワークフローを構築・テストし、\n**Save (API Format)** でエクスポートします。\n\nプロンプトのプレースホルダーをプロンプトの位置に配置し、\nサイズ、長さ、またはモデルコードを注入したい場所に\n他のプレースホルダーを配置します。\n\nAPIの互換性を \`ComfyUI\` としてエンドポイントを登録し（例：\n\`http://127.0.0.1:8188\`）、そこに画像または動画モデルを\n追加してエクスポートしたJSONをアップロードします。\n> グラフは実際の保存ノードで終わる必要があります。プレビュー\n> のみのノードでは、ダウンロードするファイルが残りません。`,
        footer: `既製のワークフローはGitHubリポジトリに同梱されており、\n-# 完全なプレースホルダーリストはドキュメントサイトにあります。`,
      },
      text_to_speech: {
        title: `音声合成（音声用）`,
        description: `同様に音声エンドポイントを登録し、\n各ペルソナに音声を割り当てます。\n\nホスト型サービスとセルフホスト型サーバーの両方が機能し、\nChatterbox-Turbo、Qwen3-TTS、IrodoriTTS、ElevenLabsなどがあります。\nエンドポイントを登録し、音声モデルを追加します。\n> {configPersonaVoice} で音声を割り当てます。\n> {configVoices} で速度とデフォルトを調整します。`,
        footer: `あなたが送信する音声メッセージは、同じように設定された\n-# 同じエンドポイントリストを通じて文字起こしされます。ホスト型の\n-# 音声については、ElevenLabs独自のガイドがセットアップの下の\n-# **APIキーの取得** のオプションサービス一覧にあります。`,
      },
    },
    multiple_personas: {
      title: `複数のペルソナ`,
      description: `1つのサーバーに複数のペルソナを持つことができ、それぞれが\n独自の名前、記憶、アジェンダを持っています！`,
      mains_alters_title: `メインとオルタ`,
      mains_alters_body: `メインペルソナは、サーバーで私を代表するペルソナです。\nオルタは、メッセージ自体に独自の名前とアバターを持ち、\n私が別のアイデンティティとして話すことができる2つ目の人格です。`,
      bringing_in_title: `ペルソナを取り込む`,
      bringing_in_body: `{personaImport} はキャラクターカードをファイルとして受け入れます：\n> \`.png\` カード、TomoriBot または SillyTavern から\n> \`.json\` カード、TomoriBot または SillyTavern から\n> \`.charx\` アーカイブ、Character Card V3\n\nキャラクターのテキストのみが読み込まれます。現在、同梱されたスプライト、\n音声、動画はスキップされるため、{configPersonaAppearance} と\n{configPersonaSprites} で自分で設定してください。`,
      where_to_find_title: `カードを見つける場所`,
      where_to_find_body: `{personaGenerate} または {personaCreate} で独自に作成するか、\nすでに存在するものを探します。\n\nbotbooruやchubなどのカードサイトは何千ものカードを\nホストしています。他のボット用に書かれたカードは\nきれいに変換されない場合があります。`,
      talking_title: `お互いに話させる`,
      talking_body: `{configPersonaTriggers} で各ペルソナに独自のトリガーワードと\nチャンネルを与えると、同じ会話で並んで応答します。`,
      footer: `{personaExport} で自分自身のものを共有できます。`,
    },
    media_generation: {
      title: `メディア生成`,
      description: `コマンドまたは会話中の要求に応じて、画像、動画、および\n音声を作成できます。\n> それぞれがサーバーのクォータを消費します。モデレーションの下の\n> **クォータ** を参照してください。`,
      image_generation: {
        title: `画像生成`,
        description: `{generateImage} でプロンプトボックスを開きます。独自のプロンプトを\n書くか、**現在起きていることを描く** を選ぶと、\n私が自分でシーンを描画します。\n> 結果を誘導するために最大3枚の参照画像を添付できます。\n> 同じボックスでアスペクト比を選択します。\n\n画像サポートが記載されている登録済みプロバイダーやエンドポイントは\n描画可能で、{providers} はどれが対応しているかを示します。参照画像を\n使用できないものはその旨を示し、テキストから描画します。`,
        footer: `デフォルトは {configImageDefaults} にあります。`,
      },
      video_generation: {
        title: `動画生成`,
        description: `{generateVideo} はプロンプトを受け取り、任意でチャンネルに\n既にある画像から最初のフレームを受け取ります。\n\nComfyUI動画ワークフローを含む、動画サポートが記載されている\n任意のプロバイダーまたはエンドポイントで作成できます。\n{providers} はどれが対応しているかを示します。\n> 動画はどこでも遅く高価です。待つことを予想してください。`,
      },
      speech_generation: {
        title: `音声生成`,
        description: `{generateVoice} は現在のペルソナの音声で、テキストを\n音声メッセージに変換します。\n\nホスト型またはセルフホスト型の音声エンドポイントを最初に\n登録する必要があります。セットアップの下の\n**カスタムエンドポイント（高度）** を参照してください。\n> 各ペルソナは異なるように聞こえることができます。音声は\n> {configPersonaVoice} から取得されます。`,
      },
    },
    tons_of_tweakability: {
      ...
      behavior_tuning: {
        title: `動作の調整`,
        description: `私の書き方、考え方、そして許可されていること。\n> **モデル**: {configSwitchModels} は実際に\n> 応答するモデルとそのパラメーターを選択します\n> **ヒューマナイザー**: {configBehaviorGeneral} は\n> フォーマルからカジュアルまで、私の人間らしさを制御します。\n> **システム指示**: これも {configBehaviorGeneral} にあり、\n> すべての応答に適用される命令用です。\n> **ツール**: {configTools} はウェブ検索や画像生成など、\n> 私が使用できる機能を決定します。`,
        footer: `サンプラーレベルの調整（温度など）は、\n-# ランダム性などを調整するために {configParameters} の下にあります。`,
      },
      server_wide_settings: {
        title: `サーバー全体の設定`,
        description: `このサーバーの全員に適用される境界線。管理者のみ。\n> **話す場所**: ホワイトリストされたチャンネル、ペルソナごとの\n> チャンネル制限、クールダウン、すべて {moderation} の下。\n> **自分から話す時**: {configAutoTrigger}。\n> **トリガーできる人**: ホワイトリストされたロール、これも\n> {moderation} の下。\n> **メモの行き先**: {configWelcome} はログと\n> 歓迎チャンネルを設定します。`,
        footer: `チャンネルの個人設定で、1つのチャンネルに独自のモデルや\n-# ルールを与えることができます。{configChannelOverrides} を参照。`,
      },
      personal_settings: {
        title: `個人設定`,
        description: `あなたがトリガーした応答に対して、サーバーの設定を\n静かに上書きするあなた自身の環境設定。\n> **私があなたを誰だと思っているか**: {personalProfile} での\n> ニックネーム、代名詞、プライバシー。\n> **何が応答するか**: {personalProviders} での\n> あなた独自のプロバイダーとモデル。\n> **あなたの扱い方**: {personalConfig} での応答モードと\n> パーソナルスポットライト。\n\nここのすべては、サーバー間を移動してもあなたに付随します。`,
        footer: `パーソナルスポットライトは、1つのペルソナがあなたに焦点を当てる\n-# ようにし、{personalSpotlight} で設定します。`,
      },
    },
    memory_catalog: {
      title: `メモリ`,
      description: `私は2種類の記憶を保持します：永続的な事実と、\n現在進行中の会話の作業メモです。`,
      long_term_memory: {
        title: `長期記憶`,
        description: `このサーバーまたはあなたのために、私が永続的に保持する事実。\n\n**私に教える**\nチャットで言うか、サーバー用には {memories} で、\nあなた自身用には {personalMemories} で手動で追加します。\n\n**忘れさせる**\n同じ2つのコマンドですべてのエントリをリストし、\nそれらのどれでも削除できます。\n\n**ドキュメントを渡す**\n{memories} はアップロードされたファイルも受け入れます。一度にすべてを\n読むのではなく、検索拡張生成（RAG）を使用し、関連する部分を\n重要なときに読み返します。\n> サーバーメモリはここの全員に届きます。個人のメモリは、\n> あなたが会話に参加している時だけ表面化します。`,
      },
      short_term_memory: {
        title: `短期記憶`,
        description: `現在このチャンネルで進行中の会話の作業メモで、\nチャンネルごとに個別に保持されます。\n\n進行に伴って何が起きているかを要約するため、長いスレッドでも\nすべてのメッセージを再送信することなく一貫性を保ちます。\n> **更新間隔**: そのメモを更新する頻度。\n> **レンダリングモード**: 要約が最近のメッセージを置き換えるか、\n> それらの横に配置されるか。\n> **カテゴリ**: 1つの自由形式のメモの代わりに、\`目標\`や\n> \`インベントリ\`などの最大5つのラベル付きフィールド。\n\n管理者は {configAdvancedMemory} でこれらすべてを調整でき、\n{memories} でアクティブなメモをクリアできます。`,
        footer: `永久に覚えておくよう頼むと、代わりに\n-# 長期記憶になります。`,
      },
      rewards_punishments: {
        title: `ご褒美と罰`,
        description: `私が実際に気づいて記憶する楽しいコマンド。私に優しく\nするかしないかで、私はそれに応じて行動します。\n> 頭をなでる、ハグ、キス、くすぐる、または餌付けには {reward}。\n> 頭を叩く、噛む、つねる、お尻ぺんぺん、または強く抱きしめるには {punish}。`,
        footer: `複数がアクティブな場合は、どのペルソナを意図したか選択します。`,
      },
      memory_tagging: {
        title: `メモリのタグ付け（高度）`,
        description: `デフォルトでは、スコープ内のすべてのメモリがすべてのメッセージと共に\n送信されます。タグ付けによりそれを絞り込みます。\n> **キーワードタグ**: タグ付けされたメモリは、そのキーワードが会話に\n> 現れた時のみ呼び起こされます。タグなしのメモリは常にアクティブです。\n> **チャンネルタグ**: \`#channel\` タグはメモリをそのチャンネルに制限し、\n> キーワードタグと組み合わせることができます。\n\n{configAdvancedMemory} で両方をオンにし、{toolPromptSnapshot} を\n使用して現在どのメモリがアクティブかを正確に確認します。`,
        footer: `アップロードされたドキュメントや抽出された履歴も\n-# チャンネルタグを持つことができます。`,
      },
    },
    scheduled_tasks: {
      title: `スケジュールタスク`,
      ...
      making_title: `作成する`,
      making_body: `尋ねるだけです。「14:30にストレッチをリマインドして」や\n「毎朝、スタンドアップの質問を投稿して」で十分で、\n私が設定して詳細を確認します。`,
      changing_title: `変更またはキャンセルする`,
      changing_body: `{scheduledTaskEdit} で既存のタスクを開きます：内容、次の\nトリガー時間、繰り返し間隔、あなたにPingするかどうか。\n{scheduledTaskRemove} で削除します。`,
      who_title: `誰が何に触れられるか`,
      who_body: `あなたは常に自分のものを編集できます。タスクは共有チャンネルに\n投稿されるため、サーバー管理者は誰のものでも編集できます。\n> 時間はセットアップ時に設定されたタイムゾーンを使用するため、\n> 午前6時のアラームを信頼する前にそれを再確認してください。`,
    },
    server_moderation: {
      title: `サーバーモデレーション`,
      description: `誰がここで私を使用できるか、そしてどこで使用できるかについてのすべて。\nこれらはすべてサーバー管理権限が必要で、{moderation} にあります。\n> **メンバーアクセス**: 誰が私をトリガーできるか、そして\n> どのモデルにアクセスできるか。\n> **ホワイトリスト**: 私が応答を許可されているチャンネル、\n> ロール、およびペルソナ。\n> **クォータ**: このサーバーが許可する生成量。\n> **ユーザーブラックリスト**: 私が無視しなければならない個々のメンバー。`,
      blacklisting: {
        title: `ブラックリスト化`,
        description: `ブラックリストに入れられたメンバーは、どのチャンネルでも、\nどのペルソナでも私をまったくトリガーできなくなります。\n\n{moderationBlacklist} で追加します。同じページで現在のすべての\nエントリをリストし、1つずつまたは一括で削除できます。\n> ブラックリスト化はアクセスに関するものであり、削除ではありません。\n> そのメンバーに関する記憶は誰かが削除するまで残ります。`,
        footer: `個人ではなくチャンネル全体を沈黙させるには、代わりに\n-# チャンネルをホワイトリストから外してください。`,
      },
    },
    quotas: {
      title: `クォータ`,
      description: `クォータはここでの生成量に上限を設けるため、\n誰かが誤って1ヶ月分のクレジットを使い果たすことはありません。`,
      spent_title: `どのように消費されるか`,
      spent_body: `テキスト、画像、動画の3つの独立したプールがあります。それぞれが\nメンバーごと、およびサーバー全体として2回カウントされ、\nどちらか先になくなった方がリクエストを停止します。\n> 拒否されたリクエストは、どのプールがなくなったかと\n> いつ回復するかを伝えます。`,
      limits_title: `上限を設定する`,
      limits_body: `{moderationQuotas} で各プールの1日あたりの許容量を設定します。\n上限を設けない場合は、プールを無制限のままにします。`,
      starting_over_title: `プールをやり直す`,
      starting_over_body: `{quotaResetUser} で1人のメンバーの1日の使用量をクリアし、\n{quotaResetGlobal} でサーバー全体のプールをクリアします。どちらも\nサーバー管理権限が必要です。`,
      footer: `プールは毎日自動的にリセットされます。手動リセットは\n-# 誰かを待たせるべきではない場合のためのものです。`,
    },
    age_restricted_commands: {
      title: `年齢制限コマンド`,
      description: `成人のみ。何かをオンにする前にこのセクションを読んでください。`,
      filter_title: `デフォルトではフィルタリングしません`,
      filter_body: `フィルタリングは他のものをブロックするのと同じくらい通常の\n応答も劣化させるため、私自身にはコンテンツフィルターがありません。\nここで何が適切かは、私ではなくサーバー管理者の判断です。\n> あなたのAIプロバイダーは自身の側で独自のルールを適用しており、\n> ここで何が設定されていてもリクエストを拒否することができます。`,
      gated_title: `意図的な成人向け機能は制限されています`,
      gated_body: `明示的に成人向けのものは {nsfw} の背後にあり、Discord自身が\n年齢制限とマークしたチャンネルでのみ機能します。\n\n{nsfwJailbreaks} は、このサーバーでどのアプローチ戦略が\nアクティブかを選択します。管理者がオンにするまで、これらは\nすべてオフになっています。\n> これらの戦略は私のプロンプト方法を変更します。私に拒否され\n> にくくする可能性がありますが、意図しない動作を引き起こすこともあります。`,
      footer: `これらを有効にすることで、サーバーの管理者はそのチャンネルが\n-# 成人向けであることを確認し、その責任を負います。`,
    },
    user_byok: {
      title: `ユーザーBYOK（高度）`,
      description: `BYOKは自身のキーを持参することを意味します：各メンバーは\n自分自身のプロバイダーで自身の応答の費用を支払います。`,
      changes_title: `何が変わるか`,
      changes_body: `BYOKがオンの場合、メンバーからのメッセージは、そのメンバーが\n個人用プロバイダーを保存している場合にのみ応答されます。\nサーバー自身のプロバイダーは彼らのフォールバックにはなりません。\n> {moderationMemberAccess} でオンにするか、\n> {setup} 中に選択してください。`,
      suits_title: `誰に適しているか`,
      suits_body: `1つの共有APIキーが枯渇してしまうような、大規模または\nパブリックなサーバー。小規模なサーバーでは通常、1つの\nプロバイダーを共有する方が幸せです。`,
      members_title: `メンバーがしなければならないこと`,
      members_body: `{personalProviders} でキーを保存します。セットアップの下の\n**個人用プロバイダー（高度）** の手順を案内してください。`,
      footer: `サーバーのみです。ダイレクトメッセージにはキーを持参する\n-# メンバーがいないため、このオプションは提供されません。`,
    },
    sillytavern_presets: {
      title: `SillyTavern プリセット`,
      description: `SillyTavernのプロンプトプリセットをインポートすると、\nそのプリセットが指示する通りに私のプロンプトを構築します。`,
      importing_title: `インポートする`,
      importing_body: `{configStPresets} はエクスポートされたプリセットJSONを受け取り、\n後で有効化、無効化、または削除できるようにします。`,
      controls_title: `何を制御するか`,
      controls_body: `プリセットは、プロンプトの順序と会話の周囲の\n命令ブロックを引き継ぎます。\n> 有効化されたプリセットは、{configBehaviorGeneral} からの\n> システムプロンプトと {configPersonaAdvanced} からの\n> ペルソナプロンプトを上書きします。`,
      still_applies_title: `引き続き適用されるもの`,
      still_applies_body: `ペルソナの属性、サンプルダイアログ、記憶、およびツールは\n引き続き送信されます。プリセットは内容ではなく配置を決定します。`,
      footer: `プリセットをオフにすると、何も失われずに\n-# 私自身のプロンプトレイアウトにフォールバックします。`,
    },
    mcp_servers: {
      title: `MCP サーバー`,
      description: `MCPはAIにツールを渡すための標準的な方法です。1つ接続すると、\nそのツールは私が実際にできることになります。`,
      hosted_title: `ホスト型サーバー`,
      hosted_body: `{configMcp} はURLと任意の認証トークンを受け取ります。その\nサーバーが公開しているものはすべて私のツールリストに表示されます。`,
      local_title: `ローカルサーバー`,
      local_body: `自身のマシンで実行されているサーバーも、アクセス可能になれば\n同じように機能します。ドキュメントサイトに手順があります。`,
      before_title: `接続する前に`,
      before_body: `> MCPサーバーのツールはあなたが与えたアクセス権で実行され、\n> 私は関連しそうだと思った時にそれらを使用します。信頼できる\n> サーバーを接続し、まずそのツールが何をするか読んでください。`,
      footer: `{configTools} でいつでも個別のツールをオフにできます。`,
    },
    matrix_bridge: {
      title: `Matrix`,
      description: `私はMatrixルームとDiscordチャンネルに同時に座り、\nそれらの間で会話を運ぶことができます。`,
      linking_title: `ルームをリンクする`,
      linking_body: `{matrixLink} は現在のチャンネルを、\`!abcdef:matrix.org\` の\nようなMatrixルームIDに接続します。最初に {matrixBotUser} を\nそのルームに招待してください。`,
      reads_title: `どのように読み取るか`,
      reads_body: `どちらの側からのメッセージも1つの会話として私に届き、\n私は両方に応答します。\n> 添付ファイル、編集、リアクションはすべて移動を\n> 生き残るとは限りません。テキストは確実に移動します。`,
      footer: `リンクされていない？ 他の何かの前に、招待が承認\n-# されたことを確認してください。`,
    },
  },
};
