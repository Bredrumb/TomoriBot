  ...
  memory: {
    ...
    history: {
      ...
      import: {
        ...
        debug_description: `各ウィンドウのプロンプトをJSONファイルでDMに送信します（インキャラクターモード専用）。`,
        debug_dm_title: `インキャラクター抽出デバッグダンプ`,
        debug_dm_description: `**{persona_name}** の **{window_count}** 個の抽出ウィンドウのプロンプトをキャプチャしました。添付されたJSONファイルには、各ウィンドウの完全な合成システムプロンプト、ユーザープロンプト、抽出エントリが含まれています。`,
        debug_dm_failed_title: `デバッグダンプをDMできませんでした`,
        debug_dm_failed_description: `デバッグダンプをDMできませんでした — DMが閉じている可能性があります。代わりにエフェメラルメッセージとして添付します。`,
