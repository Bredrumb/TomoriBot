...
export default {
  tools: {
    ...
    image: {
      ...
      notice_default_positive_tags_line: `デフォルトのポジティブ画像タグを適用しています。`,
      notice_default_negative_tags_line: `デフォルトのネガティブ画像タグを適用しています。`,
      notice_image_tags_tip_line: `ヒント: \`/persona image-tags\` または \`/personal image-tags\` でPhysical Appearanceタグを保存すると、キャラクターの一貫性が上がります。`,
      notice_persona_image_tags_detected_line: `ペルソナ画像タグを検出しました: {names}。`,
      notice_user_image_tags_detected_line: `ユーザー画像タグを検出しました: {names}。`,
