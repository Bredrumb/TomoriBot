  ...
  try {
    ...
    let updateRows = await sql<Array<{ tomori_config_id: number }>>`
			...
			SET
				...
				deliberate_tool_context_turns = COALESCE(${config.deliberate_tool_context_turns ?? null}, deliberate_tool_context_turns),
				deliberate_tool_triggers = COALESCE(${deliberateToolTriggersJson}::jsonb, deliberate_tool_triggers, '{}'::jsonb),
				fast_regeneration_enabled = COALESCE(${config.fast_regeneration_enabled ?? null}, fast_regeneration_enabled, false),
				fast_regeneration_retry_enabled = COALESCE(${config.fast_regeneration_retry_enabled ?? null}, fast_regeneration_retry_enabled, false),
				fast_regeneration_continue_enabled = COALESCE(${config.fast_regeneration_continue_enabled ?? null}, fast_regeneration_continue_enabled, false),
