import type { AnyThreadChannel, Client, Message, Sticker, Embed, Webhook } from "discord.js";
import {
...
import { getLinkedMatrixRoom, sendMatrixTypingIndicator } from "@/utils/bridges/matrix";
...
import type { TomoriState, ServerEmojiRow, ServerStickerRow, FallbackEntry } from "@/types/db/schema";
...
import { sendWebhookErrorEmbed } from "@/utils/chat/responseEmitter";
import {
...
import {
  ...
  truncateForSystemContext,
} from "@/utils/chat/helpers/contextDirectives";
import {
  acquireChannelLockForTurn,
  cleanupTextQuotaTriggerStates,
  enqueueBusyChannelMessage,
  getLastRespondedPersonaId,
...
import type { ChatInvocation, ManualTriggerInvoker, TextQuotaSource } from "@/utils/chat/invocation";
import {
  cacheUserImpersonationWebhook,
  getCachedImpersonatedUserIdForWebhook,
  normalizeAvatarUrlForMatch,
  normalizeIdentityName,
  resolveImpersonatedIdentity,
 ...
 */
export async function runChatTurn(invocation: ChatInvocation): Promise<void> {
  await runChatTurnWithLegacyArgs(
    invocation.client,
    invocation.message,
    invocation.isFromQueue,
    invocation.isManuallyTriggered,
    invocation.forceReason,
    invocation.reasoningQuery,
    invocation.llmOverrideCodename,
    invocation.isStopResponse,
    invocation.retryCount,
    invocation.skipLock,
    invocation.reminderRecipientID,
    invocation.reminderData,
    invocation.selectedPersonaId,
    invocation.isPersonaJob,
    invocation.isUserImpersonation,
    invocation.impersonatedUserId,
    invocation.textQuotaSource,
    invocation.textQuotaTriggerKey,
    invocation.textQuotaUserDiscId,
    invocation.manualSystemPrompt,
    invocation.manualPrefill,
    invocation.naiContinuationPrefill,
    invocation.emptyResponseFinishReason,
    invocation.injectedContextItems,
    invocation.forcedMentions,
    invocation.manualTriggerInvoker,
    invocation.manualStreamingContextOverrides,
  );
}

async function runChatTurnWithLegacyArgs(
  client: Client,
  message: Message,
  isFromQueue: boolean,
...
*/
  ...
  if (!skipLock) {
    ...
    lockEntry = getOrCreateChannelLockEntry(channelLockId, serverDiscId);
    releaseStaleChannelLockIfExpired(channelLockId, lockEntry);
    ...
    if (lockEntry.isLocked) {
      ...
      if (earlyTomoriState) {
        ...
        const earlyAccessState = guild
          ? await evaluateChatAccess({
              isStopResponse: !!isStopResponse,
              isDMChannel,
              isManuallyTriggered,
              isSelfMessage: isLikelySelfMessage,
              isAutochatOverride: isAutochatOverrideChannel(earlyTomoriState.config, earlyEffectiveChannelId),
              guildDiscId: guild.id,
              fallbackUserDiscId: message.author.id,
              message,
              memberRoleDiscIds: message.member?.roles.cache.map((role) => role.id),
              parentChannelId: earlyParentChannelId,
              effectiveChannelId: earlyEffectiveChannelId,
              serverId: earlyTomoriState.server_id,
        ...
        if (earlyAccessState.rejectedByWhitelist) {
          return;
        }
        ...
        if (
          ...
          shouldBotReply(message, modifiedEarlyTomoriStateForCheck, earlyAllPersonas, {
            personalAutoTriggerPersonaId: earlyAccessState.personalSpotlightStatus?.autoTriggerPersonaId ?? null,
            allowedPersonaIds: earlyAccessState.allowedPersonaIds,
          })
        ) {
          ...
          if (!isStopResponse && !message.author.bot && !message.webhookId) {
            ...
            const rejectedByCooldown = await rejectOnMessageTriggerCooldown({
              serverDiscId: guild?.id ?? message.author.id,
    ...
    acquireChannelLockForTurn(lockEntry, {
      ...
      isPersonaJob,
      selectedPersonaId,
      isCommandTriggered: !!manualTriggerInvoker,
    });
  }
  // --- End Semaphore Logic ---
  ...
  try {
    try {
      ...
      const directTriggerValidation = await validateDirectChatTrigger({
        client,
        message,
        guild,
        allPersonas,
      ...
      if (!directTriggerValidation.shouldContinue) {
        return;
      }
      if (!tomoriState) {
        return;
      }
      const isReplyToBot = directTriggerValidation.isReplyToBot;
      const replyPersona = directTriggerValidation.replyPersona;
      const isBotMentioned = directTriggerValidation.isBotMentioned;
      if (directTriggerValidation.isUserImpersonation) {
        isUserImpersonation = true;
        impersonatedUserId = directTriggerValidation.impersonatedUserId;
      }
      ...
      const accessState = await evaluateChatAccess({
        ...
        message,
        memberRoleDiscIds,
        parentChannelId: msgParentChannelId,
        effectiveChannelId: msgEffectiveChannelId,
      ...
      const { whitelistStatus, personalSpotlightStatus, allowedPersonaIds } = accessState;
      ...
      const textQuotaAdmission = await checkTextQuotaForAdmission({
        shouldApplyTextQuota,
        isPersonaJob,
        triggerKey: effectiveTextQuotaTriggerKey,
        serverId: tomoriState.server_id,
        userDiscId: effectiveTextQuotaUserDiscId,
        channel,
        locale,
      });
      if (!textQuotaAdmission.allowed) {
        return;
      }
      textQuotaStateForTrigger = textQuotaAdmission.state;
      ...
      for (let personaIndex = 0; personaIndex < personasToRespond.length; personaIndex++) {
        ...
        if (lockEntry) {
          setActiveChannelTurnState(lockEntry, {
            ...
            followUpEligible: currentPersona.tomori_id !== undefined,
            isUserImpersonation,
            impersonatedUserId,
          });
        ...
        try {
          ...
          if (reminderData && (reminderRecipientID || reminderData.self_reminder)) {
            ...
            if (isSelfReminder) {
              ...
              reminderContent = `[System: A reminder you set earlier for @${matrixLocalpart} (Mention ID: @{${matrixLocalpart}}) has triggered. Reminder: "${reminderData.reminder_purpose}". Focus on reminding and pinging @${matrixLocalpart} about this.]`;
              if (reminderData.reminder_lateness) {
                reminderContent += `\n[System: You are also ${reminderData.reminder_lateness} late in reminding the user.]`;
              }
            } else {
              reminderContent = `[System: A reminder you set earlier for <@${reminderRecipientID}> (Mention ID: ${reminderRecipientID}) has triggered. Reminder: "${reminderData.reminder_purpose}". Focus on reminding and pinging <@${reminderRecipientID}> about this.]`;
              if (reminderData.reminder_lateness) {
                reminderContent += `\n[System: You are also ${reminderData.reminder_lateness} late in reminding the user.]`;
              }
            }
            ...
            try {
              ...
              switch (streamResult.status) {
                case "completed": {
                  ...
                  resetChannelFollowUpCount(channel.id);
                ...
                case "empty_response": {
                  ...
                  if (retryCount < MAX_EMPTY_RESPONSE_RETRIES) {
                    ...
                    return await runChatTurnWithLegacyArgs(
                      client,
                      message,
                      isFromQueue,
                ...
                case "follow_up_interrupt": {
                  ...
                  incrementChannelFollowUpCount(channel.id);
    ...
    if (!skipLock && lockEntry) {
      releaseChannelLockAndReplayQueue({
        ...
        handleStopResponse,
        processQueuedMessage: async (nextMessageData) => {
          await runChatTurnWithLegacyArgs(
            client,
            ...
            nextMessageData.isStopResponse,
            0,
            false,
            undefined,
            undefined,
            ...
            nextMessageData.isUserImpersonation,
            nextMessageData.impersonatedUserId,
            nextMessageData.textQuotaSource ?? "user",
            nextMessageData.textQuotaTriggerKey,
            nextMessageData.textQuotaUserDiscId,
            nextMessageData.manualSystemPrompt,
            nextMessageData.manualPrefill,
            undefined,
            undefined,
 ...
 */
async function handleStopResponse(originalStopMessage: Message, client: Client): Promise<void> {
  try {
    log.info(
