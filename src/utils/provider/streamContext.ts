...
export function buildStreamContext(params: BuildStreamContextParams): StreamContext {
  ...
  return {
    ...
    // 8. Progress and output hooks shared by watchdogs/regeneration.
    onStreamProgress: streamingContext?.onStreamProgress,
    recordTurnOutputMessage: streamingContext?.recordTurnOutputMessage,

    // 9. Opaque message ID map for snowflake ID abstraction in LLM-visible text
