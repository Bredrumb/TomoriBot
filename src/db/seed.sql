...
DROP TABLE IF EXISTS seed_previous_persona_presets;
CREATE TEMP TABLE seed_previous_persona_presets AS
SELECT
  persona_preset_name,
  persona_preset_desc,
  ...
  COALESCE(
    preset_lineage_id,
    CASE
      WHEN lower(COALESCE(preset_avatar_path, '')) LIKE '%default.png' THEN 4
      WHEN lower(COALESCE(preset_avatar_path, '')) LIKE '%bratty.png' THEN 716
      WHEN lower(COALESCE(preset_avatar_path, '')) LIKE '%gloomy.png' THEN 1770
      WHEN lower(COALESCE(preset_avatar_path, '')) LIKE '%shy.png' THEN 3585
      WHEN lower(COALESCE(preset_avatar_path, '')) LIKE '%blind.png' THEN 50
      WHEN lower(persona_preset_name) LIKE '%bratty%' OR persona_preset_name LIKE '%メスガキ%' THEN 716
      WHEN lower(persona_preset_name) LIKE '%gloomy%' OR persona_preset_name LIKE '%ダウナー%' THEN 1770
      WHEN lower(persona_preset_name) LIKE '%shy%' OR persona_preset_name LIKE '%シャイ%' THEN 3585
      WHEN lower(persona_preset_name) LIKE '%professional%' OR persona_preset_name LIKE '%廃盤%' THEN 50
      WHEN lower(persona_preset_name) LIKE '%default%' OR persona_preset_name LIKE '%デフォルト%' THEN 4
      ELSE NULL
    END
  ) AS preset_lineage_id
...
-- match the stored baseline.
WITH preset_sync_candidates AS (
  SELECT
    p.persona_id,
    ...
    jsonb_build_object(
      ...
      'persona_prompt', COALESCE(old_pp.persona_preset_desc, pp.persona_preset_desc)
    ) AS base_snapshot,
    (
      CASE
        WHEN persona_preset_array_starts_with(p.attribute_list, COALESCE(old_pp.preset_attribute_list, pp.preset_attribute_list))
          OR (
            pc.persona_prompt IS NOT DISTINCT FROM COALESCE(old_pp.persona_preset_desc, pp.persona_preset_desc)
            AND persona_preset_array_starts_with(
              p.attribute_list,
              array_prepend(
                '{bot}''s Description: ' || COALESCE(old_pp.persona_preset_desc, pp.persona_preset_desc),
                COALESCE(old_pp.preset_attribute_list, pp.preset_attribute_list)
              )
            )
          ) THEN 4
        ELSE 0
      END +
      CASE
        WHEN persona_preset_array_starts_with(p.sample_dialogues_in, COALESCE(old_pp.preset_sample_dialogues_in, pp.preset_sample_dialogues_in))
          AND persona_preset_array_starts_with(p.sample_dialogues_out, COALESCE(old_pp.preset_sample_dialogues_out, pp.preset_sample_dialogues_out)) THEN 4
        ELSE 0
      END +
      CASE
        WHEN persona_preset_array_starts_with(COALESCE(pc.trigger_words, ARRAY[]::TEXT[]), COALESCE(old_pp.preset_trigger_words, pp.preset_trigger_words)) THEN 2
        ELSE 0
      END +
      CASE
        WHEN pc.persona_prompt IS NOT DISTINCT FROM COALESCE(old_pp.persona_preset_desc, pp.persona_preset_desc) THEN 1
        ELSE 0
      END
    ) AS content_score,
    CASE
      WHEN pp.preset_language = COALESCE(s.registration_locale, '') THEN 3
      WHEN pp.preset_language = split_part(COALESCE(s.registration_locale, ''), '-', 1) THEN 2
      WHEN pp.preset_language = 'en-US' THEN 1
      ELSE 0
    END AS locale_score
  FROM personas p
  ...
  LEFT JOIN seed_previous_persona_presets old_pp
    ON old_pp.preset_lineage_id = pp.preset_lineage_id
    AND old_pp.preset_language = pp.preset_language
  LEFT JOIN persona_preset_sync_state ps ON ps.persona_id = p.persona_id
  WHERE pp.preset_lineage_id IS NOT NULL
    AND ps.persona_id IS NULL
    AND (
      pp.preset_lineage_id = p.persona_lineage_id
      OR persona_preset_array_starts_with(p.attribute_list, pp.preset_attribute_list)
      OR persona_preset_array_starts_with(p.attribute_list, COALESCE(old_pp.preset_attribute_list, pp.preset_attribute_list))
      OR (
        pc.persona_prompt IS NOT DISTINCT FROM pp.persona_preset_desc
        AND persona_preset_array_starts_with(
          p.attribute_list,
          array_prepend('{bot}''s Description: ' || pp.persona_preset_desc, pp.preset_attribute_list)
        )
      )
      OR (
        pc.persona_prompt IS NOT DISTINCT FROM COALESCE(old_pp.persona_preset_desc, pp.persona_preset_desc)
      ...
      OR (
        persona_preset_array_starts_with(p.sample_dialogues_in, pp.preset_sample_dialogues_in)
        AND persona_preset_array_starts_with(p.sample_dialogues_out, pp.preset_sample_dialogues_out)
        AND persona_preset_array_starts_with(COALESCE(pc.trigger_words, ARRAY[]::TEXT[]), pp.preset_trigger_words)
      )
      OR (
        persona_preset_array_starts_with(p.sample_dialogues_in, COALESCE(old_pp.preset_sample_dialogues_in, pp.preset_sample_dialogues_in))
        AND persona_preset_array_starts_with(p.sample_dialogues_out, COALESCE(old_pp.preset_sample_dialogues_out, pp.preset_sample_dialogues_out))
        AND persona_preset_array_starts_with(COALESCE(pc.trigger_words, ARRAY[]::TEXT[]), COALESCE(old_pp.preset_trigger_words, pp.preset_trigger_words))
      )
    )
),
ranked_preset_sync_candidates AS (
  SELECT
    *,
    ROW_NUMBER() OVER (
      PARTITION BY persona_id
      ORDER BY content_score DESC, locale_score DESC, preset_language ASC
    ) AS row_num
  FROM preset_sync_candidates
)
...
-- Rebase auto-synced official preset fields. Array fields update only when the
-- server's current value still starts with the previous official baseline, so
-- appended local content is preserved and edits/removals are left alone.
WITH preset_sync_targets AS (
  SELECT
    ps.persona_id,
    ps.preset_lineage_id,
    ps.preset_language,
    ps.base_snapshot,
    ...
    pp.persona_preset_desc AS new_persona_prompt
  ...
  LEFT JOIN persona_configs pc ON pc.persona_id = ps.persona_id
  WHERE ps.sync_mode = 'auto'
),
preset_sync_merged AS (
  SELECT
    persona_id,
    preset_lineage_id,
    preset_language,
    ...
    CASE
      WHEN persona_preset_array_starts_with(
        ...
        THEN persona_preset_rebase_text_array(
          current_attribute_list,
          persona_preset_snapshot_text_array(base_snapshot, 'attribute_list'),
          new_attribute_list
        )
    ...
    CASE
      WHEN persona_preset_array_starts_with(
        ...
        AND persona_preset_array_starts_with(
          current_sample_dialogues_out,
          persona_preset_snapshot_text_array(base_snapshot, 'sample_dialogues_out')
        )
        THEN persona_preset_rebase_text_array(
          current_sample_dialogues_in,
          persona_preset_snapshot_text_array(base_snapshot, 'sample_dialogues_in'),
          new_sample_dialogues_in
        )
      ELSE current_sample_dialogues_in
    END AS merged_sample_dialogues_in,
    CASE
      WHEN persona_preset_array_starts_with(
          current_sample_dialogues_in,
          persona_preset_snapshot_text_array(base_snapshot, 'sample_dialogues_in')
        ...
        THEN persona_preset_rebase_text_array(
          current_sample_dialogues_out,
          persona_preset_snapshot_text_array(base_snapshot, 'sample_dialogues_out'),
          new_sample_dialogues_out
        )
      ELSE current_sample_dialogues_out
    END AS merged_sample_dialogues_out,
    persona_preset_rebase_text_array(
      current_trigger_words,
      persona_preset_snapshot_text_array(base_snapshot, 'trigger_words'),
      new_trigger_words
    ) AS merged_trigger_words,
    CASE
      WHEN current_persona_prompt IS NOT DISTINCT FROM (base_snapshot ->> 'persona_prompt')
        THEN new_persona_prompt
      ELSE current_persona_prompt
    END AS merged_persona_prompt,
    jsonb_build_object(
      ...
      'persona_prompt', new_persona_prompt
    ) AS new_base_snapshot
  FROM preset_sync_targets
),
updated_personas AS (
  UPDATE personas p
  SET
    attribute_list = m.merged_attribute_list,
    sample_dialogues_in = m.merged_sample_dialogues_in,
    sample_dialogues_out = m.merged_sample_dialogues_out,
    updated_at = CURRENT_TIMESTAMP
  FROM preset_sync_merged m
  WHERE p.persona_id = m.persona_id
    AND (
      p.attribute_list IS DISTINCT FROM m.merged_attribute_list
      OR p.sample_dialogues_in IS DISTINCT FROM m.merged_sample_dialogues_in
...
updated_persona_configs AS (
  ...
  SELECT
    persona_id,
    merged_trigger_words,
    merged_persona_prompt
  FROM preset_sync_merged
  ON CONFLICT (persona_id) DO UPDATE
  SET
    trigger_words = EXCLUDED.trigger_words,
    persona_prompt = EXCLUDED.persona_prompt
  WHERE persona_configs.trigger_words IS DISTINCT FROM EXCLUDED.trigger_words
    OR persona_configs.persona_prompt IS DISTINCT FROM EXCLUDED.persona_prompt
  RETURNING persona_id
)
UPDATE persona_preset_sync_state ps
SET
  base_snapshot = m.new_base_snapshot,
  last_synced_at = CURRENT_TIMESTAMP
FROM preset_sync_merged m
WHERE ps.persona_id = m.persona_id
  AND ps.sync_mode = 'auto'
  AND ps.base_snapshot IS DISTINCT FROM m.new_base_snapshot;
