...
Just a minor update that allows you to give your personas Sprites which are basically alternative avatars for your bot based on the expression (or identity) they wish to show for their response. It doesn't require any subagent so it doesn't cost you anything in terms of latency *and* tokens, but it still requires you to upload Sprites through `/persona sprites`. 

If your persona has atleast *one* Sprite uploaded, a new block in context is added as a guide for usage, for example:
```
